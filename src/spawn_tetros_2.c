...
void			S_tetro_spawn_2(t_tetris *tetris)
{
...
void			J_tetro_spawn_2(t_tetris *tetris)
{
	...
	{
		tetris->curr_tetro[0][i] = 1;
		tetris->curr_tetro[1][i] = 0;
	}
	tetris->curr_tetro[1][2] = 1;
	tetris->curr_len_x = 3;
	tetris->curr_len_y = 2;
	if (tetris->old_rotation == 1 || tetris->old_rotation == 3)
	{
...
void			L_tetro_spawn_2(t_tetris *tetris)
{
	...
	for (int i = 0; i < 2; i++)
		tetris->curr_tetro[i] = (int*)malloc(sizeof(int) * 3);
	for (int j = 0; j < 3; j++)
			tetris->curr_tetro[0][j] = 0;
	for (int k = 0;k < 3; k++)
			tetris->curr_tetro[1][k] = 1;
	tetris->curr_tetro[0][2] = 1;
	tetris->curr_len_x = 3;
	tetris->curr_len_y = 2;
	if (tetris->old_rotation == 3 && tetris->act_x > 0)
		tetris->act_x -= 1;
...
void			get_current_tetro_2(t_tetris *tetris, int tetro)
{
	...
	tetro_pt[0] = &(S_tetro_spawn_2);
	tetro_pt[1] = &(I_tetro_spawn_2);
	tetro_pt[2] = &(J_tetro_spawn_2);
	tetro_pt[3] = &(L_tetro_spawn_2);
	tetro_pt[5] = &(T_tetro_spawn_2);
	tetro_pt[6] = &(Z_tetro_spawn_2);
